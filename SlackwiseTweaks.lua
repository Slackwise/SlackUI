...
package.preload["static-data"] = package.preload["static-data"] or function(...)
...
package.preload["core"] = package.preload["core"] or function(...)
  ...
  local function getSpecName()
    ...
    if specIndex then
      ...
      local _let_11_ = GetSpecializationInfo(specIndex)
      local specID = _let_11_[1]
      local specName = _let_11_[2]
  ...
  local function canCollectTransmog(itemInfo)
    if isClassic() then
      ...
      local _let_15_ = C_TransmogCollection.GetItemInfo(itemInfo)
      local itemAppearanceID = _let_15_[1]
      local sourceID = _let_15_[2]
      if sourceID then
        local _let_16_ = C_TransmogCollection.GetAppearanceSourceInfo(sourceID)
        local _ = _let_16_[1]
        local _0 = _let_16_[2]
        local _1 = _let_16_[3]
        local _2 = _let_16_[4]
        local isCollected = _let_16_[5]
        local _3 = _let_16_[6]
        local _4 = _let_16_[7]
        local _5 = _let_16_[8]
        local _6 = _let_16_[9]
  ...
  local function sellGreyItems()
    for bag = 0, NUM_BAG_SLOTS do
      for slot = 0, C_Container.GetContainerNumSlots(bag) do
        ...
        if link then
          local _let_19_ = C_Item.GetItemInfo(link)
          local _ = _let_19_[1]
          local itemLink = _let_19_[2]
          local itemQuality = _let_19_[3]
  ...
  local function isSpellKnown(spellName)
    local _let_23_ = GetSpellLink(spellName)
    local _ = _let_23_[1]
    local spellID = _let_23_[2]
    if spellID then
  ...
  local function getKeyByValue(targetTable, targetValue)
    ...
    __fnl_global__each_2dpair({key, value, targetTable}, _25_())
    return nil
  end
  ...
  local function findFirstElement(targetTable, kvPredicate)
    ...
    __fnl_global__each_2dpair({key, value, targetTable}, _26_())
    return {nil, nil}
  end
  local function findElements(targetTable, kvPredicate)
    ...
    __fnl_global__each_2dpair({key, value, targetTable}, _27_())
    return found
  end
  ...
  local function groupBy(array, groupingFunction)
    local grouped = {}
    local function _33_()
      local _let_32_ = groupingFunction(value)
      local groupingKey = _let_32_[1]
      local newValue = _let_32_[2]
    ...
    __fnl_global__each_2dpair({key, value, array}, _33_())
    return grouped
  end
  local function findLargestIndex(array)
    ...
    __fnl_global__each_2dpair({key, value, array}, _35_())
    return largestIndex
  end
  ...
  local function foundDruidRare(name)
    ...
    local _let_48_ = GetGameTime()
    local currentHour = _let_48_[1]
    local currentMinute = _let_48_[2]
...
require("core")
package.preload["bindings"] = package.preload["bindings"] or function(...)
  ...
  local function bindBestUseItem(bestItemMap)
    ...
    local function _52_(item)
      return bestItemMap[item.itemID], {item.itemID, item.stackCount}
    end
    itemsByBestStrength = groupBy(containerItemInfos, _52_)
...
package.preload["options"] = package.preload["options"] or function(...)
  ...
  options = {type = "group", args = {enable = {name = "Enabled", desc = ("Enable/disable " .. addonName), type = "toggle", get = _73_, set = _74_, order = 0}, debug = {name = "Enable Debugging", desc = "Enable debugging logs and stuff", type = "toggle", get = _76_, set = _77_}, bind = {type = "execute", name = "Set Bindings", desc = "Set binding presets for current character's class and spec.", func = _79_, hidden = true}, reset = {type = "execute", name = "Reset All Data", desc = "DANGER: Wipes all settings! Cannot be undone!", func = _80_, confirm = true}}}
  return nil
end
require("options")
package.preload["mount"] = package.preload["mount"] or function(...)
  ...
  local function mount()
    ...
    do
      ...
      _94_ = cond(isAlternativeMountRequested(), _95_, IsInGroup(), _96_, "else", _97_())
    end
    local _98_
    do
      mountByUsage("FLYING")
    ...
    return cond(IsMounted(), _92_, UnitUsingVehicle("player"), _93_, IsOutdoors(), cond((isActuallyFlyableArea() and not IsSubmerged()), _94_, IsSubmerged(), cond(isAlternativeMountRequested(), _98_, "else", _99_()), "else", cond(IsInGroup(), cond(isAlternativeMountRequested(), _100_, "else", _101_()), "else", cond(isAlternativeMountRequested(), _102_, "else", _103_()))))
  end
  return mount
end
require("mount")
package.preload["slackwise"] = package.preload["slackwise"] or function(...)
